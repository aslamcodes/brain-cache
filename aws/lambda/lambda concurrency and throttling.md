...
---
Concurrency can be controlled as Lambda can scale up to 1000 concurrent invokation

Throttle behavior
	Synchronous invocation - returns ThrottleError
	Asynchronous invocation - DLQ (needs to be configured), retry up to 6 hours at exponentially from 1 second to 5 seconds. By default, Lambda retries a failed asynchronous invocation up to two times

Concurrently Issue
- If one function in the account goes over the limit, the other functions possibly get throttled
- It is recommended reserve (limit) concurrency for expected usage scenarios
